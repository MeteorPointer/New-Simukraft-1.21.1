...
| `BuildingStructureIds` | `location/matches` | 建筑文件名对应的原版结构 ID |
| `BuildingStructurePort` | `structureId/findAt` | 用已建成建筑回答原版结构查询，供 FTB Quests 结构任务使用 |
...
- FTB Quests 的“寻找结构”任务不读 SQLite。建筑包里的 `.sk` 会登记成数据包结构 `simukraft:<分类>/<文件名>`，例如 `simukraft:residential/lumberjacks_house`。分类标签是 `#simukraft:residential`、`#simukraft:commercial`、`#simukraft:industry`、`#simukraft:public`、`#simukraft:other`，全部建筑是 `#simukraft:placed_buildings`。玩家站在已完成建筑占地内，或贴着外墙、门口、屋顶一格内，才算进入该结构。脚的位置在包围盒外一格时也会算上。新增或更换建筑包后需要 `/reload` 或重启，任务编辑器里才会出现新结构。这些结构没有结构集，不会自然生成。
